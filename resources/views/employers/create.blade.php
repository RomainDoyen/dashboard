      ...
      <div class="app-card-body">
        <form class="settings-form" method="POST" action="{{route('employer.store')}}">
          ...
          <div class="mb-3">
            ...
            <select name="departement_id" id="departement_id" class="form-control">
              ...
              @foreach($departements as $departement)
                <option value="{{$departement->id}}" value="{{old('departement_id')}}">
                  {{$departement->name}}
                </option>
              @endforeach
            </select>
          ...
          <div class="mb-3">
            ...
            <input type="text" class="form-control" id="setting-input-2" placeholder="Entrer le prénom" name="prenom" value="{{old('prenom')}}" required>
            @error('prenom')
              <div class="text-danger">{{ $message }}</div>
            @enderror
          </div>
          <div class="mb-3">
            ...
            <input type="email" class="form-control" id="setting-input-3" placeholder="Entrer l'email" value="{{old('email')}}" name="email">
            @error('email')
              <div class="text-danger">{{ $message }}</div>
            @enderror
          </div>
          ...
          <div class="mb-3">
            ...
            <input type="number" class="form-control" id="setting-input-3" placeholder="Entrer le montant journalier" name="montant_journalier" value="{{old('montant_journalier')}}">
